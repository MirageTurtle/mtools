...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert vmess:// URL to sing-box format JSON")
    parser.add_argument(
        "vmess_url",
        type=str,
        help="The vmess:// URL string to convert"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
...
def vmess_to_singbox(vmess_config: Dict[str, Any], tag: str = "vmess-proxy") -> Dict[str, Any]:
    """
    ...
    if network == "ws":
        ...
        transport = {
            "type": "grpc",
            "service_name": path if path else ""
        }
    elif network == "quic":
        transport = {
            "type": "quic"
